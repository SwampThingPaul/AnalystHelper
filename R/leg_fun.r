...
#' @export
#' @return This function provides a custom categorical or continuous legend. Good for raster plots
...
leg.fun <- function(b,pal,leg.title,
  ...
  if (length(pal) != n.bks) {
    warning("Length of pal does not match n.bks; recycling colors")
    pal <- rep(pal, length.out=n.bks)
  }
  ...
  title.x <- if(is.null(title.x)){mid.val}else{title.x}
  title.y <- if(is.null(title.y)){top.val}else{title.y}
